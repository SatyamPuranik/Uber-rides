...
@dp.table(
    name="silver_ride_events",
    table_properties={"quality": "sliver"},
    comment="Filter and enriched ride events. Invalis rows dropped; quality metrics tracked."
)
...
@dp.expect_or_drop("valid_status", "status IN ('completed', 'cancelled_by_driver','cancelled_by_user', 'no_driver_found', 'in_progress' )")
@dp.expect_or_drop("valid_vehicle", "vehicle_type IN ('Sedan', 'SUV', 'Mini', 'Auto', 'Bike')")
